...
class Layer:
    ...
    def printf_exp_value_header_file(self):
        ...
        file_name = 'output_' + self.name + '.h'

        f = open(file_name, 'w')
        f.write('// This file is automatically generated\n')

        f.write('#ifndef OUTPUT_' + self.name.upper() + '_H_\n')
        f.write('#define OUTPUT_' + self.name.upper() + '_H_\n')
        f.write('#include <stdint.h>\n')

        #f.write(" int8_t " + self.name + "[] = {\n")
        f.write(" int8_t __attribute__((section(\".xheep_data_interleaved\"))) output_" + self.name + "[] = {\n")
        ...
        f.write('#endif //OUTPUT_' + self.name.upper() + '_H_\n')
        f.close()

    def printf_weights_header_file(self):

        ctype = "int8_t"
        file_name = self.weights.name + '.h'

        f = open(file_name, 'w')
        f.write('// This file is automatically generated\n')

        f.write('#ifndef ' + self.weights.name.upper() + '_H_\n')
        f.write('#define ' + self.weights.name.upper() + '_H_\n')

        f.write('#include <stdint.h>\n')

        f.write('#define ' + self.weights.name.upper() + '_ROW_ ' + str(self.weights.rows) + '\n')
        f.write('#define ' + self.weights.name.upper() + '_COL_ ' + str(self.weights.cols) + '\n')

        f.write('#define ' + self.weights.name.upper() + '_SIZE_ ' + str(self.weights.rows*self.weights.cols) + '\n')


        f.write('#define INPUT_LAYER_' + str(id) + '_SIZE_ ' + str(np.prod(self.input.shape)) + '\n')
        f.write('#define OUTPUT_LAYER_' + str(id) + '_SIZE_ ' + str(np.prod(self.output.shape)) + '\n')

        f.write(" " + ctype + " __attribute__((section(\".xheep_data_interleaved\"))) " + self.weights.name + "_w[] = {\n")

        for row in self.weights.matrix:
            for value in row[:-1]:
                f.write('%d,' % (value))
            f.write('%d,\n' % (row[-1]))
        ...
        f.write('#define ' + self.weights.name.upper() + '_BIAS_SIZE_ ' + str(np.prod(self.weights.bias.shape)) + '\n')

        f.write(" int32_t __attribute__((section(\".xheep_data_interleaved\"))) " + self.weights.name + "_b[] = {\n")

        for value in self.weights.bias[:-1]:
            f.write('%d,' % (value))
        f.write('%d,\n' % (self.weights.bias[-1]))

        f.write('};\n\n')

        f.write(" int32_t __attribute__((section(\".xheep_data_interleaved\"))) " + self.weights.name + "_mean = " + str(self.weights.mean)+ ";\n")
        f.write(" int32_t __attribute__((section(\".xheep_data_interleaved\"))) " + self.weights.name + "_scale = " + str(self.weights.scale)+ ";\n")

        f.write('#endif //' + self.weights.name.upper() + '_H_\n')
        f.close()
...
for W in weight_matrices:
    ...
    O.printf_weights_header_file()
    O.printf_exp_value_header_file()
